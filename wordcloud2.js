    ...
    var drawText = function drawText (gx, gy, info, word, weight, distance, theta, rotateDeg, attributes, extraDataArray) {
      ...
      elements.forEach(function (el) {
        if (el.getContext) {
          ...
          var span = document.createElement(attributes?.tag ?? 'span')
          var transformRule = ''
          ...
          var size = (fontSize * info.mu)
          if (size < 12) {
            transformRule += ` scale(${size / 12})`
          }
          var styleRules = {
            position: 'absolute',
            display: 'block',
            font: (fontWeight) + ' ' +
              Math.max(12, size) + 'px ' + settings.fontFamily,
            left: ((gx + info.gw / 2) * g + info.fillTextOffsetX) + 'px',
            top: ((gy + info.gh / 2) * g + info.fillTextOffsetY) + 'px',
            width: info.fillTextWidth + 'px',
            height: info.fillTextHeight + 'px',
            lineHeight: Math.max(fontSize, 12 / info.mu) + 'px',
            whiteSpace: 'nowrap',
